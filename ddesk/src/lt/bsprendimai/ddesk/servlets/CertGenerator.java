...
public class CertGenerator extends HttpServlet {
	...
	try {
	    ...
	    UserHandler uh = (UserHandler) request.getSession().getAttribute(
		    "userHandler");
	    ClientAccessor ca = (ClientAccessor) request.getSession()
		    .getAttribute("clientAccessor");
		...
		RequestDispatcher rd = request
			.getRequestDispatcher("/intranet/generation.jsp");
	    ...
	    InputStreamReader rd = new InputStreamReader(CertGenerator.class
	    ...
	    rd = new InputStreamReader(CertGenerator.class
		    .getResourceAsStream("/desk.crt"));
	    ...
	    attrs.put(X509Principal.OU, request
		    .getParameter("X509Principal.OU"));
	    attrs.put(X509Principal.L, request.getParameter("X509Principal.L"));
	    attrs.put(X509Principal.CN, request
		    .getParameter("X509Principal.CN"));
	    attrs.put(X509Principal.EmailAddress, request
		    .getParameter("X509Principal.EmailAddress"));
	    ...
	    v3c.setNotAfter(new Date(System.currentTimeMillis()
		    + (1000L * 60 * 60 * 24 * 30)));
	    ...
	    store.setKeyEntry(request.getParameter("X509Principal.T"), kp
		    .getPrivate(), null, new Certificate[] { cert, root });
	    ...
	    byte[] pwdMD5 = Hex.encode(MessageDigest.getInstance("MD5").digest(
		    cert.getEncoded()));
	    ...
	    response.setHeader("Content-disposition", "inline;filename="
		    + request.getParameter("X509Principal.T").trim() + ".p12");

	    OutputStream out = response.getOutputStream();
	    store.store(out, request.getParameter("X509Principal.PWD").trim()
		    .toCharArray());
	    out.close();
