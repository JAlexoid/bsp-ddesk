...
public class ClientAccessor implements Serializable {
	...
	if (ChangeDetector.safeEquals(companyId, this.companyId))
	    return;
	...
	try {
	    this.selected = (Company) SessionHolder.currentSession().getSess()
		    .createQuery(
			    " FROM " + Company.class.getName()
				    + " m WHERE m.id = ?").setInteger(0,
			    companyId).setMaxResults(1).uniqueResult();
	    this.contract = (CompanyContract) SessionHolder.currentSession()
		    .getSess().createQuery(
			    " FROM " + CompanyContract.class.getName()
				    + " k WHERE k.company = ? ").setInteger(0,
			    companyId).setMaxResults(1).uniqueResult();
	...
	if (!userHandler.isOwner())
	    return;
	if (ChangeDetector.safeEquals(personId, this.personId))
	    return;
	...
	try {
	    this.person = (Person) SessionHolder
		    .currentSession()
		    .getSess()
		    .createQuery(
			    " FROM " + Person.class.getName() + " WHERE id = ?")
		    .setInteger(0, personId).uniqueResult();
	    for (Entry<Integer, String> c : ParameterAccess.getLanguages()
		    .entrySet()) {
		if (c.getValue().equals(person.getLanguage()))
	...
	try {
	    return (List<Company>) SessionHolder.currentSession().getSess()
		    .createQuery(
			    "from " + Company.class.getName()
	...
	try {
	    return (List<Person>) SessionHolder
		    .currentSession()
		    .getSess()
		    .createQuery(
			    "from "
				    + Person.class.getName()
				    + " t WHERE t.company = ? AND t.company >= 0 AND t.id <> 0 ORDER BY t.id ")
		    .setInteger(0, this.companyId).list();
	} catch (Exception ex) {
	    // SessionHolder.endSession();
	    // Messenger.addFatalKeyMessage("error.transaction.abort",userHandler.getUserLocale());
	    ex.printStackTrace();
	...
	try {
	    String message;
	    if ((userHandler.getUser().getLoginLevel() == null || userHandler
		    .getUser().getLoginLevel() != 0)
		...
		String password = new BigInteger(1, MessageDigest.getInstance(
			"MD5").digest(pwd2.getBytes())).toString(16);
		person.setPassword("");
	    ...
	    String lang = ParameterAccess.getLanguages().get(getLanguage())
		    .toLowerCase();
		...
		int i = (Integer) sess.createQuery(
			"SELECT count(*) FROM " + Person.class.getName()
				+ " e WHERE e.loginCode = ? ").setString(0,
			person.getLoginCode()).iterate().next();

		if (i > 0) {
		    message = UIMessenger.getMessage(userHandler.getUserLocale(),
			    "application.login.loginExists");
		    UIMessenger.addErrorMessage(message, "");
		    return StandardResults.FAIL;
		} else {
		    this.person.setLoginChange(false);
	...
	try {
	    ...
	    String password = new BigInteger(1, MessageDigest
		    .getInstance("MD5").digest(pwd2.getBytes())).toString(16);
	    person.setPassword("");
	    if (password.length() < 32) {
		for (int i = (32 - password.length()); i > 0; i--) {
		    password = "0" + password;
	...
	if (!userHandler.isOwner())
	    return StandardResults.FAIL;
	try {
	    List<CertificateEntry> lce = (List<CertificateEntry>) SessionHolder
		    .currentSession().getSess().createQuery(
			    "FROM " + CertificateEntry.class.getName()
				    + " WHERE person = ? AND valid = true ")
		    .setInteger(0, getPersonId()).list();
	...
	if (!userHandler.isOwner())
	    return StandardResults.FAIL;
	try {
	    Person cmp = (Person) SessionHolder.currentSession().getSess().get(
		    Person.class,
		    (Serializable) this.getPersonIdParameter().getValue());
	...
	if (ret.equals(StandardResults.SUCCESS)) {
		...
		UIMessenger.addFatalKeyMessage("error.transaction.abort",
			userHandler.getUserLocale());
		String message = UIMessenger.getMessage(userHandler
			.getUserLocale(), "application.clients.unsaved");
		UIMessenger.addErrorMessage(message, "");
	    }
	    this.selected = new Company();
	    this.contract = new CompanyContract();
	    ...
	    UIMessenger.addFatalKeyMessage("error.transaction.abort", userHandler
		    .getUserLocale());
	    String message = UIMessenger.getMessage(userHandler.getUserLocale(),
		    "application.clients.unsaved");
	    UIMessenger.addErrorMessage(message, "");
	}
	return ret;
    }
	...
	if (!userHandler.isOwner())
	    return StandardResults.FAIL;
	// System.out.println("Slected ID: "+selected.getId());
	String ret = selected.update();

	pwd1 = selected.getNotes();
	if (ret.equals(StandardResults.SUCCESS)) {
	    // System.out.println("LIDaste: "+contract.getLastInvoiceDate());
	    ret = contract.update();
	    if (!ret.equals(StandardResults.SUCCESS)) {
		SessionHolder.endSession();
		UIMessenger.addFatalKeyMessage("error.transaction.abort",
	...
	if (!userHandler.isOwner())
	    return StandardResults.FAIL;
	try {
	    Company cmp = (Company) SessionHolder.currentSession().getSess()
		    .get(Company.class,
			    (Serializable) clientIdParameter.getValue());
	    SessionHolder.currentSession().getSess().delete(cmp);
	    SessionHolder.currentSession().getSess().flush();
		...
		la = (List<Company>) SessionHolder
			...
			.createQuery(
				"from "
					...
					+ " t  WHERE t.id = ? OR t.id = 0 ORDER BY t.id ")
	...
	try {
	    la = (List<Person>) SessionHolder
		    ...
		    .createQuery(
			    "from "
				    + Person.class.getName()
				    + " t WHERE t.company = 0 AND t.id <> 0 ORDER BY t.id ")
		    .list();
	} catch (Exception ex) {
	    // SessionHolder.endSession();
	    // Messenger.addFatalKeyMessage("error.transaction.abort",userHandler.getUserLocale());
	...
	try {
	    la = (List<Person>) SessionHolder
		    ...
		    .createQuery(
			    "from "
				    + Person.class.getName()
				    + " t WHERE (t.loginLevel = ? ) AND t.id <> 0 ORDER BY t.id ")
		    .setInteger(0, Person.PARTNER).list();
	} catch (Exception ex) {
	    // SessionHolder.endSession();
	    UIMessenger.addFatalKeyMessage("error.transaction.abort", userHandler
		    .getUserLocale());
	...
	si.setLabel(UIMessenger.getMessage(this.userHandler.getUserLocale(),
		"pplication.person.loginLevel.admin"));
	ll.add(si);

	si = new SelectItem();
	si.setValue(new Integer(Person.OTHER));
	si.setLabel(UIMessenger.getMessage(this.userHandler.getUserLocale(),
		"pplication.person.loginLevel.other"));
	ll.add(si);

	si = new SelectItem();
	si.setValue(new Integer(Person.PARTNER));
	si.setLabel(UIMessenger.getMessage(this.userHandler.getUserLocale(),
		"pplication.person.loginLevel.partner"));
	ll.add(si);
