...
public class UserHandler implements Serializable {
	...
	try {
	    ...
	    char[] pwdMD5 = Hex.encodeHex(MessageDigest.getInstance("MD5")
	    ...
	    Query q = SessionHolder
		    ...
		    .createQuery(
			    " FROM "
				    ...
				    + "  WHERE lower(loginCode) = lower(?) AND password = ? ");
		...
		String message = UIMessenger.getMessage(this.userLocale,
		...
		if (user.getCompany() == Company.OWNER
			|| user.getLoginLevel() == Person.PARTNER) {
		    return StandardResults.INTRANET;
		} else {
		    company = (Company) SessionHolder.currentSession()
			    .getSess().createQuery(
				    " FROM " + Company.class.getName()
					    + "  WHERE id = ?").setInteger(0,
				    user.getCompany()).uniqueResult();
		    contract = (CompanyContract) SessionHolder.currentSession()
			    .getSess().createQuery(
	...
	try {
	    String password = new BigInteger(1, MessageDigest
		    .getInstance("MD5").digest(originalPwd.getBytes()))
		    .toString(16);
		...
		if (!pwd1.equals(pwd2)) {
		    message = UIMessenger.getMessage(this.userLocale,
		...
		password = new BigInteger(1, MessageDigest.getInstance("MD5")
			.digest(pwd1.getBytes())).toString(16);
		...
		message = UIMessenger.getMessage(this.userLocale,
			"application.login.wrongPassword");
		UIMessenger.addErrorMessage(message, "");
		return StandardResults.FAIL;
	    }
	    return StandardResults.SUCCESS;
	...
	try {
	    ByteArrayInputStream bos = new ByteArrayInputStream(Base64
		    .decodeBase64(this.user.getSearchFilter().getBytes()));
	...
	if (user.getLoginLevel() == Person.PARTNER)
	    return true;
	else
	    return false;
    }
